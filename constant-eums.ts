...
export const getCellColor = (
  ...
  const selfValue = selfVals[col] ?? 0;
  const defaultValue = defaultVals[col] ?? 0;

  const isChanged = selfValue !== defaultValue;
  const isFilled = selfValue >= row;

  if (isChanged && isFilled) return CellColor.Yellow;   // Violet if changed
  if (!isChanged && isFilled) return CellColor.Yellow_light;       // Yellow if default
  return CellColor.White;                                     // White otherwise
};
